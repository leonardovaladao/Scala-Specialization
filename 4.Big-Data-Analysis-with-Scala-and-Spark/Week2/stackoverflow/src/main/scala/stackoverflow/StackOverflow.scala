...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.values.map( x => (x.head._1, x.map(_._2)) ).map({ case (q, a) => (q, answerHighScore(a.toArray)) } )
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = {
      ...
      vectors.map(v => (findClosest(v, means), v)).groupByKey().mapValues(averageVectors).collect().toMap
    }
    val newMeans: Array[(Int, Int)] = means.map(oldMean => newMeansMap(oldMean)) // compute the new means of each group of points
    val distance = euclideanDistance(means, newMeans) // compute a Map that associate the old `means` values to their new values
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = (mostCommonLang._2 / vs.size.toDouble) * 100.0
